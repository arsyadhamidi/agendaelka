...
@section('content')
    <div class="row">
        <div class="col-lg">
            <form action="{{ route('dosen-rps.update', $rps->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card">
                    <div class="card-header">
                        <a href="{{ route('dosen-rps.rps', $rps->tahun_id) }}" class="btn btn-primary">
                                    ...
                                    <select name="tahun_id" class="form-control @error('tahun_id') is-invalid @enderror"
                                        ...
                                        @foreach ($tahuns as $tahun)
                                            <option value="{{ $tahun->id }}"
                                                {{ $rps->tahun_id == $tahun->id ? 'selected' : '' }}>
                                                {{ $tahun->tahun ?? '-' }}</option>
                                        @endforeach
                                    </select>
                                    @error('tahun_id')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="matkul_id" class="form-control @error('matkul_id') is-invalid @enderror"
                                        ...
                                        @foreach ($matkuls as $data)
                                            <option
                                                value="{{ $data->id }}"{{ $rps->matkul_id == $data->id ? 'selected' : '' }}>
                                                {{ $data->matkul ?? '-' }}</option>
                                        @endforeach
                                    </select>
                                    @error('matkul_id')
